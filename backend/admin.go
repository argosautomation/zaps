...
func HandleListKeys(rdb *redis.Client) fiber.Handler {
	return func(c *fiber.Ctx) error {
		keys, err := services.ListAPIKeys(rdb)
		if err != nil {
			return c.Status(500).JSON(fiber.Map{"error": "Redis error"})
		}

		// Transform for Admin UI if needed, or return direct
		// Admin UI expects specific format with "prefix"
		var response []map[string]interface{}
		for _, k := range keys {
			// Prefix logic
			displayPrefix := "gk_???..."
			if len(k.Key) > 12 {
				displayPrefix = k.Key[:12] + "..."
			}
			...
			response = append(response, map[string]interface{}{
				...
				"created": k.CreatedAt,
				"enabled": k.Enabled,
				"key_id":  services.RedisKeyPrefix + k.Key, // Internal Redis key for Deletion
			})
		}
...
func HandleGenerateKey(rdb *redis.Client) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err := services.StoreAPIKey(rdb, meta); err != nil {
